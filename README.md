...
* 50일차
    * 인턴십 마지막 주
    * RNN 모델로 주식성장률을 예측할때 학습후 일주일 또는 한달 단위로 예측을 할때 Input이 어떤모양인지 알아야 해결할 수 있을 것같다.
    * Paul 과 얘기를 해봤는데 내가 생각했던대로 예측된 Y값을 다시 예측에 사용하는 건 어떻냐고 한다.
    * 그런 함수가 있으면 내가 벌써 찾았겠지요... 내가 못찾는건가

* 51일차
    * 주식 성장률을 예측하는 다른 코드를 확인하고 쭉 따라해봤다.
    * 이 코드에서 아이디어를 얻어서 정규화된 값을 그대로는 아니지만 비슷하게라도 되돌리는데에 성공했다.
    * 두 코드를 합치면 뭐가 연성될것 같긴 한데

* 52일차
    * Predict 함수에서 예측된 값을 추출하는 방법은 없는것 같아서 직접 만들기로 했다.
    * 첫번쨰 윈도우에 일주일치의 값을 넣고 하루치를 예측하여 list에 넣는다.
    * list에 들어간 예측값을 일주일치의 다음 값(예측할 날)의 방문자수에 넣는다.
    * 슬라이딩 윈도우처럼 다음값을 한줄 씩 가져오며 예측함수에 돌린다.
    * 일단 예측값을 뽑아내고 그래프로 나타내는데에 성공했다.
    * 이제 이걸 배열에 삽입하기만 하면 프로젝트가 끝난다.
    * 끝!!!!! 성공이다!!!!!!!!!! 이제 코드를 다듬기만 하면 된다

* 53일차
    * 예측할 기간을 지정할 수 있게 코드를 수정
    * 깔끔하게 코드를 정리했다
    * 정규화된 데이터를 돌려놓는 코드를 추가해야한다

* 54일차(인턴십 마지막)
    * 데이터 최대 최소값과 분포도가 달라서 그런지 정규화된 데이터를 돌려놓아도 이상한 결과가 나온다
    * 2시에 프로젝트 발표
    * 지나가던 강아지도 앉아서 발표를 듣고갔다. 땡큐 하비
    * 끝끝끄끄튺트끝끄ㅡㄲㅌ끄ㅡㅌㄲ트끝끄
