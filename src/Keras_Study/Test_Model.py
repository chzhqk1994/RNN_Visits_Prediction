...
def preprocess_data(dataset, seq_len):
    amount_of_features = len(dataset.columns)  # 피쳐 수 = 4
    ...
    row = round(0.9 * result.shape[0])  # 여기서 곱해지는 숫자 (0 ~ 1.0 가 데이터셋을 학습, 테스트로 나누는 비율이다
    print('row : ', row)
...
window = 20  # 시퀀스 길이로 넘겨진다, 타임 스탬프 비슷한건가? 이전의 5개의 값을 보고 다음 값 1개를 예측하는 방식
...
model = build_model([X_train.shape[2], window, 100, 1])  # Feature 의 수(input_dim), 윈도우의 크기, ??,마지막 출력 output_dim 을 넘긴다

# 텐서보드와 연동
# tensorboard --logdir= 그래프 파일 경로
# tb_hist = keras.callbacks.TensorBoard(log_dir='./graph', histogram_freq=0, write_graph=True, write_images=True)
# model.fit(X_train, y_train, batch_size=20, nb_epoch=1000, validation_split=0.1, verbose=2, callbacks=[tb_hist])

model.fit(X_train, y_train, batch_size=20, epochs=200, validation_split=0.1, verbose=2)  # callbacks는 텐서보드에 사용
